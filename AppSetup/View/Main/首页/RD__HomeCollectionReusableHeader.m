...
- (void) initializeWithView {
    ...
    self.lqs__backgroundColor = @"R";
    
    
    UIImageView* imageView = [UIImageView lqs__image:@"home_bg_banner"];
    ...
    UILabel* titleLabel = [UILabel lqs__text:@"" textColor:@"#fff" fontSize:20];
    ...
    UILabel* orderLabel = [UILabel lqs__text:@"" textColor:@"#fff" fontSize:30];
    ...
    UILabel* promptOrderLabel = [UILabel lqs__text:@"我的排名" textColor:@"#fff" fontSize:14];
...
- (void) setupWithItem:(LQSCollectionSectionItem *)item dataSourceDictionary:(NSDictionary *)dataSourceDictionary {
    ...
    if ([dataSourceDictionary lqs__isNSDictionary] == NO) {
        return;
    }
    
    self.titleLabel.text = [NSString stringWithFormat:@"距考试%@天", [dataSourceDictionary lqs__toStringWithObjectForKey:@"examination"]];
    
    NSString* tempText = @"999";
    NSString* tempText2 = [dataSourceDictionary lqs__toStringWithObjectForKey:@"orderValue"];
