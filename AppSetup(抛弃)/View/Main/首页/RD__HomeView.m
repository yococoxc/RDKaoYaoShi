...
- (void) lqsRequestImplement:(LQSRequestImplement *)request success:(id)result {
    ...
    /*************************************************************************/
    if ([request.tag lqs_isEqualToString:@"HomeUserOrderDetailsRequest"]) {
        if ([result lqs_isNSDictionary]) {
    ...
    /*************************************************************************/
    if ([request.tag lqs_isEqualToString:@"HomeActityRecommendListRequest"]) {
        if ([result lqs_isNSDictionary]) {
            NSString* code = [result lqs_toStringWithObjectForKey:@"code"];
            if ([code lqs_isEqualToString:@"1"]) {
                NSDictionary* data = [result objectForKey:@"data"];
                if ([data lqs_isNSDictionary] == NO) {
                    return;
                }
                NSArray* list = [data objectForKey:@"list"];
                if ([list lqs_length] == 0) {
                    return;
                }
    ...
    /*************************************************************************/
    if ([request.tag lqs_isEqualToString:@"HomeActityListRequest"]) {
        if ([result lqs_isNSDictionary]) {
            NSString* code = [result lqs_toStringWithObjectForKey:@"code"];
            if ([code lqs_isEqualToString:@"1"]) {
                NSDictionary* data = [result objectForKey:@"data"];
